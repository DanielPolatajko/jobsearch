...
class CompanyCareerCrawler:
    ...
    def extract_companies_from_lists(self) -> list[dict[str, Any]]:
        ...
        list_pages = self.company_list_page_repository.get_unscraped_company_list_pages(
            limit=self.max_lists, location_relevant=self.location is not None
        )
    ...
    def extract_homepages(self) -> dict[str, str]:
        ...
        if not self.companies:
            self.logger.warning(
                "No companies found. Run extract_companies_from_lists() first."
            )
            return {}

        self.logger.info(f"Extracting homepages for {len(self.companies)} companies")
    ...
    def find_career_pages(self) -> dict[str, str]:
        """
        Find career pages from the company homepages using crawl4ai.
        ...
        Returns:
            Dictionary mapping company names to their career page URLs
        """
        if not self.homepages:
            self.logger.warning(
                "No company homepages found. Run extract_homepages() first."
            )
            return {}

        self.logger.info(f"Finding career pages for {len(self.homepages)} companies")

        try:
            for company_name, homepage_url in self.homepages.items():
                # Create a crawler to find links from the homepage
                documents = asyncio.run(self.crawl_webpage(homepage_url))

                # Extract all links from the homepage
                all_links = []
                for doc in documents:
                    if doc.links:
                        all_links.extend([l["href"] for l in doc.links["external"]])

                # Find the career page URL
                # If location or target roles are provided, prioritize specific career pages
                career_page_url = self._find_career_page_url(
                    homepage_url, all_links, self.location, self.target_roles
                )

                if career_page_url:
                    self.career_pages[company_name] = career_page_url
                    self.logger.info(
                        f"Found career page for {company_name}: {career_page_url}"
                    )
                else:
                    self.logger.warning(
                        f"Could not find career page for {company_name}"
                    )

            return self.career_pages
